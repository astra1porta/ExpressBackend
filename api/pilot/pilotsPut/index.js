...
module.exports = makeInjectable(
  ...
  async function ({ knex }, req, res) {
    ...
    if (!req.body.crewId || req.body.crewId === null) {
      return res
        .status(406)
        .json({ message: "Request body missing 'crewId' parameter." });
    }
    if (!Number.isInteger(req.body.crewId)) {
      return res
        .status(403)
        .json({ message: "Request body 'crewId' parameter not an Integer." });
    }
    ...
    const update = await knex("Pilots")
      .where({crewId: req.body.crewId})
      .update({
